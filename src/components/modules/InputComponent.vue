<template>
  <div class="flex justify-center items-center">
    ...
    <div v-if="isTextFieldType">
      <!-- 通常のテキスト入力 -->
      <input :id="type" v-model="inputValue" type="text"
    ...
    <div v-else-if="type === 'age'">
      <input id="age" v-model.number="inputValue" type="number" min="0"
        class="w-[240px] h-[40px] border rounded-[6px] focus:outline-none focus:border-black px-2"
    ...
    <div v-else-if="type === 'date'">
      ...
      <input id="date" v-model="formattedDate" type="text" readonly
        class="w-[240px] h-[40px] border rounded-[6px] focus:outline-none focus:border-black cursor-pointer px-2"
        :placeholder="placeholder || '生年月日'" @click="toggleDatePicker">
      <!-- 日付ピッカーコンポーネント -->
    ...
    <div v-else-if="type === 'password'">
      <input id="password" v-model="inputValue" type="password"
        class="w-[240px] h-[40px] border rounded-[6px] focus:outline-none focus:border-black px-2"
    ...
    <div v-else-if="type === 'textarea'">
      <!-- 大きな入力領域が必要な場合 -->
      <textarea id="textarea" v-model="inputValue"
        class="w-[240px] h-[184px] border rounded-[6px] focus:outline-none focus:border-black px-2"
        :placeholder="placeholder || '詳しい内容を教えてください'"></textarea>
    </div>

    <!-- その他のフィールド (デフォルト) -->
    <input v-else :id="type" v-model="inputValue" :type="type"
      class="w-[240px] h-[40px] border rounded-[6px] focus:outline-none focus:border-black" :placeholder="placeholder">
  </div>
</template>
...
const isTextFieldType = computed(() => ["name", "email", "report"].includes(props.type));
