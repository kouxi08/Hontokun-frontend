  ...
  <div>
    ...
    <div v-if="step === 'year-month'">
      <div class="header">
        ...
        <select v-model="selectedYear" class="year-selector" @change="handleYearChange">
          <option v-for="year in yearRange" :key="year" :value="year">{{ year }}年</option>
        </select>
        <button @click="nextYear">›</button>
      </div>
      <div class="months-grid">
        <button v-for="(month, index) in months" :key="index" class="month-button" @click="selectMonth(index + 1)">
          {{ month }}
        </button>
      </div>
    </div>
      ...
      <div class="calendar">
        ...
        <div class="calendar-grid">
          <div v-for="(date, index) in calendarDates" :key="index">
            <button v-if="date" class="date-button" @click="selectDate(date)">
              {{ date }}
            </button>
...
const selectMonth = (month) => {
  selectedMonth.value = month.value.padStart(2, '0');
...
const selectDate = (date) => {
  selectedDate.value = date.value.toString().padStart(2, '0');
