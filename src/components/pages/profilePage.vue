<template>
  <div v-if="loading" class="w-screen h-screen bg-[#FFFCF8] fixed">
    <div class="w-full flex justify-between items-center pt-[32px] px-[48px]">
      ...
      <p
        class="font-zenMaru font-bold text-[24px] text-center absolute left-1/2 transform -translate-x-1/2">
        {{ user.nickname }}
      </p>
    </div>
    ...
    <div
      ...
      <Card
        v-for="cat in cats"
        :key="cat"
        :icon-name="cat.enemy.name"
        :icon-image="cat.enemy.url"
        :accuracy="cat.accuracy"
        :attempts="cat.quizSetList"
        @detail-selected="handleDetailSelected" />
    </div>
