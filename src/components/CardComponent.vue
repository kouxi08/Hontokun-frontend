<template>
  ...
  >
    <!-- アコーディオンのヘッダー -->
    <div class="flex justify-between items-center w-full h-full px-4 cursor-pointer">
      <!-- 画像 -->
      <img :src="iconImage" alt="ネコ画像" class="w-12 h-12 rounded-full object-cover" />
      <!-- 名前 -->
    ...
    <!-- アコーディオンの詳細 -->
    <div v-if="showDetails" class="w-full p-4 flex flex-col gap-4 transition-opacity duration-300">
      <!-- 詳細カードを繰り返し表示 -->
      <div
      ...
      >
        <p class="font-zenMaru text-[#808080] text-[16px] w-[80px]">
          {{ attempt.timeAgo }}
        </p>
